...
const WorkExperienceSection = ({
  ...
  return (
    <motion.section
      key={workLoading ? "work-loading" : "work-loaded"}
      variants={fadeUpVariants}
      className="space-y-8 mt-30"
    >
      <div className="flex flex-col gap-3">
        <p className="text-sm uppercase tracking-[0.3em] text-slate-500">work experience</p>
        <h2 className="text-2xl font-semibold text-slate-900 sm:text-3xl">
          ประสบการณ์ทำงานที่ผลักดันให้โปรเจ็กต์เติบโต
        </h2>
        <p className="max-w-3xl text-base text-slate-600">
          จากการออกแบบสถาปัตยกรรม ไปจนถึงการนำทีมพัฒนา นี่คือเส้นทางการทำงานที่ผสานความคิดสร้างสรรค์กับความ
          มุ่งมั่นในการส่งมอบผลลัพธ์ที่จับต้องได้
        </p>
      </div>
      ...
      {workError ? (
        ...
        >
          ไม่สามารถโหลดประสบการณ์ทำงานล่าสุดได้ กำลังแสดงข้อมูลที่มีอยู่ล่าสุดแทน
        </motion.div>
      ) : null}
            ...
            >
              ...
              <div className="relative space-y-6">
                <div className="flex items-center justify-between gap-4">
                  <div className="h-12 w-12 rounded-2xl bg-slate-100/80" />
                ...
                <div className="space-y-3">
                  ...
                  <div className="h-3 w-1/2 rounded-full bg-slate-100/60" />
                </div>
              </div>
            </motion.div>
          ))
                  ...
                  >
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                      <div className="flex items-start gap-4">
                        {item.logo ? (
                          <div className="relative h-18 w-18 overflow-hidden rounded-2xl border border-slate-200/80 bg-white/80 p-2 shadow-[0_12px_35px_-22px_rgba(15,23,42,0.45)]">
                            <img src={item.logo} alt={item.company} className="h-full w-full object-contain" loading="lazy" />
                          </div>
                        ) : (
                        ...
                        <div className="space-y-1.5">
                          <div className="flex flex-row items-center gap-2">
                            <h3 className="text-xl font-semibold text-slate-900 sm:text-2xl">{item.company}</h3>
                            <svg
                          ...
                          <p className="text-base font-medium text-slate-500">
                            {item.position}
                            {item.employmentType ? <span className="text-slate-400"> · {item.employmentType}</span> : null}
                          </p>
                      ...
                      <div className="flex flex-col items-start gap-2 text-left sm:items-end sm:text-right">
                        ...
                        {item.isCurrent ? (
                          <span className="rounded-full bg-emerald-500/90 px-3 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-white shadow-[0_10px_25px_-18px_rgba(16,185,129,0.55)]">
                            CURRENT
                          </span>
                        ) : null}
                      </div>
                    ...
                    {/* <div className="flex items-center justify-between text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">
                      ...
                      <svg
                        ...
                        viewBox="0 0 20 20"
                        fill="currentColor"
                        className={`h-4 w-4 transition ${isExpanded ? "rotate-180 text-slate-600" : "rotate-0 text-slate-400"}`}
                      >
                        <path
                          fillRule="evenodd"
                          d="M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 011.08 1.04l-4.25 4.25a.75.75 0 01-1.08 0L5.21 8.27a.75.75 0 01.02-1.06z"
                        ...
                        {item.technologies.length ? (
                          <div className="flex flex-wrap gap-2">
                            {item.technologies.map((tech) => (
                              <span
                                key={tech}
                                className="inline-flex items-center gap-2 rounded-full border border-slate-200/60 bg-white/70 px-3 py-1 text-xs font-medium text-slate-600 shadow-[0_18px_40px_-32px_rgba(15,23,42,0.6)]"
                              >
